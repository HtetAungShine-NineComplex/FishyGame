...
public class SlotWins : MonoBehaviour
{
	...
	void showWin()
	{
		...
		if (winLineOffset == -1)
		{
			Debug.LogWarning("findNextWin() returned -1, no wins found! serverWinData.Count: " + (serverWinData?.Count ?? 0));
			return;
		}
		...
		if (slot.IsMultiplayer)
		{
			...
			if (serverWinData == null || serverWinData.Count == 0)
			{
				Debug.LogError($"showWin ERROR: serverWinData is null or empty! Count: {serverWinData?.Count ?? 0}");
				Debug.LogError($"showWin ERROR: This should not happen - server data should be preserved!");
				return;
			}
			if (winLineOffset >= serverWinData.Count)
			{
				Debug.LogError($"showWin ERROR: winLineOffset {winLineOffset} >= serverWinData.Count {serverWinData.Count}");
				return;
			}
		...
		for (int i = 0; i < winSymbolCount; i++)
		{
			...
			if (i < currentWin.matches)
			{
				...
				{
					Debug.LogWarning($"showWin: No winbox prefab found for symbol index {currentWin.symbols[i].GetComponent<SlotSymbol>().symbolIndex}");
				}
			}
			else
			{
